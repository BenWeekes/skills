...
description: |
  ...
  Covers RTC Web, RTC React, RTC iOS, RTC Android, ConvoAI REST API, and TEN Framework.
...
TEN extensions run in the TEN runtime. For unit testing, extract business logic from
event handlers into pure functions testable without the runtime.

**Extract pure functions:**

```python
# ❌ Hard to test — logic is inside the runtime handler
async def on_data(self, ten_env: AsyncTenEnv, data: Data):
    text = data.get_property_string("text")
    result = text.upper() + "_processed"
    output = Data.create("output")
    output.set_property_string("result", result)
    await ten_env.send_data(output)

# ✅ Testable — pure function extracted
def process_text(text: str) -> str:
    return text.upper() + "_processed"

async def on_data(self, ten_env: AsyncTenEnv, data: Data):
    text = data.get_property_string("text")
    result = process_text(text)  # tested independently
    output = Data.create("output")
    output.set_property_string("result", result)
    await ten_env.send_data(output)
```

**Mock `TenEnv` for integration tests:**

```python
from unittest.mock import AsyncMock, MagicMock

async def test_on_data_sends_output():
    mock_ten_env = AsyncMock()
    mock_ten_env.send_data = AsyncMock()

    data = MagicMock()
    data.get_property_string.return_value = "hello"

    ext = MyExtension("test")
    await ext.on_data(mock_ten_env, data)

    mock_ten_env.send_data.assert_called_once()
```

Note: `signal.signal` and `atexit` cannot be used in TEN extensions (worker thread
constraint — see `references/ten-framework/extensions.md`). Do not test cleanup
via signal handlers.
