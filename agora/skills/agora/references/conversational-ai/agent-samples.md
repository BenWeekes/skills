...
- `AGENT_AUTH_HEADER` is now optional — backend auto-generates v007 token from `APP_ID` + `APP_CERTIFICATE` if not set
...
- `NEXT_PUBLIC_DEFAULT_PROFILE` env var overrides the client's default profile (e.g., `THYMIA`, `THYMIA_VIDEO`)
...
Voice biomarker wellness analysis via custom LLM server + agent-ui-kit ThymiaPanel:
- Enable on base clients: `NEXT_PUBLIC_ENABLE_THYMIA=true` (no separate Thymia client directories needed)
- Profiles: `THYMIA` (voice), `THYMIA_VIDEO` (video+avatar)

> **[recipes/thymia.md](https://github.com/AgoraIO-Conversational-AI/agent-samples/blob/main/recipes/thymia.md)** — Full setup guide
